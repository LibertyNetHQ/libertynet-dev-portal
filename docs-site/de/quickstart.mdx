...
## 4. Einen Knoten aufrufen und prüfen, dass er geantwortet hat

Eine Identität zu verifizieren beweist, dass DID und Schlüssel zusammenpassen. Es beweist **nicht**, dass jemand erreichbar ist, und auch nicht, dass die antwortende Seite den privaten Schlüssel hat. Dieser Schritt prüft beides.

<Warning>
**Die meisten Knoten im Netz kannst du gar nicht aufrufen.**

Viele Einträge melden private Adressen oder lokale Namen, die nur für ihren Betreiber etwas bedeuten — für ihn echt, von außen nicht erreichbar. Frag die Registry direkt nach den brauchbaren:

```bash
curl -s 'https://registry.libertynet.ai/nodes?callable=1'
```

Zurück kommen nur Knoten, die **öffentlich erreichbar sind und eine Registrierungssignatur tragen**. Heute sind das wenige, und genau diese Menge ist die Voreinstellung beider SDKs.
</Warning>

**Die Nonce bestimmst du.** Genau das macht die Antwort zu einem Beweis statt zu einer Aufzeichnung: eine Signatur über etwas vorbereitetes ließe sich einfach abspielen.
...
Jetzt die Signatur prüfen. Ed25519 braucht eine Bibliothek — der einzige Schritt im Quickstart, der etwas installiert:
...
Geht das durch, hält dieser Knoten tatsächlich den privaten Schlüssel der Identität, die du gerade verifiziert hast.

## 5. Oder in einer Zeile
